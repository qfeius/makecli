...
- `cmd/` - Cobra 子命令层（root、version、configure[token/config/set/get/verify]、app[create/init/list/info/delete/deploy]、entity、relation、record、apply、diff、update、skills[list/install/update/remove]、schema、integration[ocr]、preflight、login、daemon[隐藏，含 start/stop/restart/status/uninstall launchd 托管子命令]、agent[隐藏]）；git.go 收口共享 go-git 原语
