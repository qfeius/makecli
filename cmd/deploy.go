...
// deploy 只推 beta：production 没有本地 push 入口，只能经 app promote 从 beta 发布——
// 「先 beta 后 production」由此成为结构约束（词汇 beta ⇄ 服务端 preview 的翻译收口在 internal/api）
const envBeta = api.EnvBeta
...
func newDeployCmd() *cobra.Command {
  ...
  makecli app deploy --status --output json  # 机器可读的进度快照`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// 无 --status 时 json 不合法：auto 直接落回 table（agent 跑 deploy 也该拿到推送日志，而不是报错）
			if !status && output == outputAuto {
				output = outputTable
			}
			...
			if err := runDeploy(force); err != nil {
				return err
			}
			if !wait {
				return nil
	...
	cmd.Flags().StringVar(&output, "output", outputAuto, outputFlagUsage+"; json requires --status")
	return cmd
}

// runDeploy 编排「纯 push 部署」到 beta：读 app key → 本地 git 门控（fail-fast）→ 注册门控 → 备仓库 → 推 HEAD。
// 本地门控刻意在网络调用之前：脏工作树 / 无仓库 / 无提交都不该先白跑一趟仓库准备，
// 提示用户先 commit 即可，零网络往返。
func runDeploy(force bool) error {
...
func runDeployStatus(wait bool, timeout time.Duration, output string) error {
	...
	task, err := client.GetBuildTask(sha, api.EnvBeta)
...
func renderBuildStatus(task *api.BuildTask, url string) {
	...
	rows := []struct{ label, value string }{
		{"App:", task.AppKey},
		{"Environment:", api.DisplayEnv(task.Environment)},
