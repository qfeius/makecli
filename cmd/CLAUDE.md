...
app.go:              app 命令组，挂载 create / list / delete / deploy 子命令；提供 loadAppManifestFromFile 共享 helper（从 YAML 加载唯一 Make.App 资源）
app_create.go:       app create 子命令（合并了原 app init）——一条命令完成 本地脚手架 + 远端 App + 代码仓库。位置参数 <appKey> 同时是「目录名 + key」（deriveAppKey 取 filepath.Base(filepath.Abs(arg)) 推导，覆盖 shop/`.`/`..`，validResourceKey 把关）；--name 展示名（缺省=key，不再兼作 key），--description；执行序「远端先行」 解析key→加载凭证→assertScaffoldClear(只读存在性预检,任一存在即拒绝)→CreateApp(远端)→writeScaffold(远端成功后才写 CLAUDE.md/AGENTS.md/.gitignore/apps/dsl/app.yaml；scaffoldFile{embed,out} 映射模板名，.gitignore 让 deploy 的 git add -A 不吞 node_modules/构建产物/密钥)→prepareCodeRepos——远端失败(坏token/冲突/网络)时本地零残留,换 profile 重跑干净；prepareCodeRepos 成功静默（成功只打印 `App 'X' created successfully` 一行，仓库地址只在 deploy 关心），失败降级为 stderr 警告（deploy 幂等重试）；newAppManifest 单一构造点（脚手架与 CreateApp 共用，空 description 不进 properties 避免 diff 漂移）；renderAppDSL 用 ResourceManifest+yaml.Marshal(2空格缩进) 写出可被 apply/diff 读回的 app.yaml；-f 文件模式仅建远端不脚手架
...
deploy.go:           app deploy 子命令（`makecli app deploy`），--env preview|production（必选）选定环境后调用代码仓库服务 CreateRepository（MakeService.CreateResource，幂等）拿到对应 cloneUrl；app key 取自 apps/dsl/app.yaml（appKeyFromDSL：app 身份单一真相源，文件缺失即给可操作错误，无 --app 旋钮）；用 go-git/v5（纯 Go，不再 shell-out git，故二进制自包含无需系统装 git）执行「快照即部署」gitDeploy：openOrInitRepo（不存在则就地 PlainInit）→ snapshotWorktree（git add -A 尊重 .gitignore，工作树有变更才提交，每次部署都提交确保推的是当前工作树而非旧 HEAD）→ pushHead（CreateRemoteAnonymous 承载 cloneUrl 不落 .git/config，推 head→refs/heads/<deployBranch> 常量 dev webhook 约定，HTTP BasicAuth make:<token>，NoErrAlreadyUpToDate 视为成功）；--force 透传 PushOptions.Force；gitSignature 提交署名优先 user git 配置缺省回退 makecli 身份；gitDeployFunc 包级变量便于测试打桩
deploy_test.go:      覆盖 runDeploy 编排（deployCall 打桩 gitDeployFunc 隔离 FS/推送：preview/production 目标与 --force 透传、app.yaml 取 key、app.yaml 缺失/key 非法、非法 env、无凭证、API 错误、cloneUrl 缺失、gitDeploy 错误透传）+ gitDeploy 真 go-git（newBareRemote 本地裸仓库做 file-transport push 目标：init+commit+推 dev 分支、.gitignore 排除 secret/node_modules、redeploy 推当前工作树而非旧 HEAD、干净 redeploy 为 up-to-date no-op），enterAppDir 临时工程目录隔离 FS + httptest 隔离网络
