...
configure.go:        configure 命令组（无本地标志，复用全局 --profile；组级 PersistentPreRunE 经 config.ValidateProfileName 拒绝保留名 settings 作 profile），默认行为等同 token 子命令；子命令: token（交互写 ~/.make/credentials）/ config（交互写 ~/.make/config）/ set（直接写单个 key）/ get（读取单个 key）/ verify（在线验证 token）；validateConfigKey 校验合法 profile key 集合（meta-server-url / repo-server-url / auth-server-url / X-Tenant-ID / X-Operator-ID）；特殊键 environment 经 setEnvironment 路由到全局 [settings]（非 profile，值校验 ∈ dev/test/production，写 config.SetSetting），get environment 读 settings 缺省回退 dev
...
app_create.go:       app create 子命令——一条命令完成 本地脚手架 + 远端 App + git 仓库 + 代码仓库。位置参数 <appKey> 同时是「目录名 + key」（deriveAppKey 取 filepath.Base(filepath.Abs(arg)) 推导，覆盖 shop/`.`/`..`，validResourceKey 把关）；--name 展示名（缺省=key），--description；执行序「远端先行」 解析key→加载凭证→assertScaffoldClear(只读存在性预检,任一存在即拒绝;.gitignore 不在列由 init 幂等管理)→CreateApp(远端)→writeScaffold(远端成功后才写 CLAUDE.md/AGENTS.md/apps/dsl/app.yaml；scaffoldFile{embed,out} 映射模板名)→scaffoldGit(initGitRepo+ensureGitignore+stageAndCommit「Initial scaffold for <key>」,复用 app init 内核再加 initial commit→create 产物即干净可立即 deploy;git 失败降级 stderr 警告不阻断已成功的远端创建,全程不写 stdout 保持成功输出一行)→prepareCodeRepos——远端失败(坏token/冲突/网络)时本地零残留,换 profile 重跑干净；prepareCodeRepos 成功静默（成功只打印 `App 'X' created successfully` 一行，仓库地址只在 deploy 关心），失败降级为 stderr 警告（deploy 幂等重试）；newAppManifest 单一构造点（脚手架与 CreateApp 共用，空 description 不进 properties 避免 diff 漂移）；renderAppDSL 用 ResourceManifest+yaml.Marshal(2空格缩进) 写出可被 apply/diff 读回的 app.yaml；-f 文件模式仅建远端不脚手架
app_create_test.go:  覆盖 renderAppDSL / writeScaffold / assertScaffoldClear / runAppCreate / runAppCreateFromFile 的单元测试（DSL渲染+往返/脚手架文件不含.gitignore/预检拒绝已存在/scaffolds 干净 git 仓库带 initial commit 即可 deploy/appKey推导含'.'/成功输出简洁无仓库信息/仓库失败 stderr 警告但 create 成功/API失败本地零残留/本地已存在不调远端即拒绝/无凭证不留半成品/未知profile/文件模式/非法目录名拒绝），用 httptest 隔离网络 + t.TempDir 隔离 FS + chdir helper 测 '.' 推导 + captureStderr 测降级警告；stubRepoServer 临时指向仓库 mock；含 validResourceKey 通用 key 校验测试（长度 2-20，不可以下划线开头）
app_init.go:         app init 子命令——把普通目录变为可部署 app 仓库形态：git init（幂等）+ .gitignore 增量补齐，刻意不 commit（提交时机交还用户），不要求 apps/dsl/app.yaml；可选 [dir] 默认 cwd（对齐 preflight）；runAppInit 调 initGitRepo + ensureGitignore（git.go），逐项打印 `git:`/`​.gitignore:` 状态行；被 app create 内核复用，deploy 的 openRepo 门控依赖此命令先行建仓
app_init_test.go:    覆盖 runAppInit 的单元测试（建仓+写.gitignore+状态输出 / 幂等再跑报 already / 刻意不 commit→refs/heads 为空无分支），t.TempDir 隔离 FS + captureStdout 验证状态行
