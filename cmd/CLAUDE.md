...
configure.go:        configure 命令组（复用全局 --profile + 本地 --sample；组级 PersistentPreRunE 经 config.ValidateProfileName 拒绝保留名 settings 作 profile），默认行为等同 token 子命令，--sample 打印带注释 INI 参考模板到 stdout 即退（sampleConfig 常量：profile 覆盖键以占位值平铺展示当参考卡，直写需替换占位主机名或删行回退 preset；configure_test.go 完整性测试守护其含全部 validConfigKeys 且活跃值过真实 loader）；子命令: token（交互写 ~/.make/credentials）/ config（交互写 ~/.make/config）/ set（直接写单个 key，Long+Example 摊开 environment 魔法路由与合法 key）/ get（读取单个 key，Long+Example 对称）/ verify（在线验证 token）；validateConfigKey 校验合法 profile key 集合（meta-server-url / repo-server-url / auth-server-url / X-Tenant-ID / X-Operator-ID）；特殊键 environment 经 setEnvironment 路由到全局 [settings]（非 profile，值校验 ∈ dev/test/production，写 config.SetSetting），get environment 读 settings 缺省回退 production
...
client.go:           公共 helper，resolveProfile() 收口凭证与配置解析（前置 config.ValidateProfileName 拒绝保留名 settings），resolveEnvironment() 收口环境 preset（--env flag > [settings] environment > 默认 production，未知名报错），envName() 是其 fail-safe 姊妹（同解析链但吞 LoadSettings 错误回退默认，供错误展示回显环境名）；newClientFromProfile(...api.Option)（收变参横切选项，如 WithDryRun）构建 Meta/Data 客户端，newRepoClientFromProfile() 构建代码仓库服务客户端并额外返回裸 token（供 deploy 的 git Basic 认证）；firstNonEmpty 统一「flag > profile config > 环境 preset」取值链；withGateway 把解析出的主机基址补上网关前缀 apiGatewayPath=/api/make（幂等：完整 URL 原样返回），Meta/Repo 客户端构造与 configure verify 共用
client_test.go:      覆盖 resolveEnvironment 优先级（默认 production / settings 覆盖默认 / --env 覆盖 settings / 未知环境报错）与 withGateway 网关前缀拼接（补前缀/幂等/裁尾斜杠/空串），setEnvFlag 临时覆盖全局 Environment
...
app_init_test.go:    覆盖 runAppInit 的单元测试（完整脚手架 CLAUDE.md/AGENTS.md/app.yaml/.gitignore/.git + app.yaml key 取目录名 / 缺省 appKey 取 cwd 名 / 非法目录名拒绝 / 幂等再跑报 exists·already·complete 且保留用户编辑 / 刻意不 commit→refs/heads 为空无分支），t.TempDir 隔离 FS + chdir 测默认 cwd + captureStdout 验证状态行
...
entity_create.go:       entity create 子命令，位置参数为 Entity key，--name 为展示名（缺省回退 key），--json 加载 fields；校验 field.Key 合法（validResourceKey）；--app 传 appKey；--dry-run 经 api.WithDryRun 注入 X-Dry-Run 远端校验不落库，成功打印 would-be 行；loadFields 从 JSON 文件加载字段定义
...
preflight.go:        preflight 顶级子命令，按 --app-type（fullstack/service/ui，默认 fullstack）校验工作目录（可选位置参数 [dir]，默认 cwd）是否具备对应形态的 Make app 骨架——apps/dsl 是身份核心三形态必查（app.yaml 驻于此），service 加 apps/service/package.json、ui 加 apps/ui/package.json、fullstack 两者全要；逐项 os.Stat 打印 ✓/✗ 清单（含 Type 头部），未知 --app-type 返回普通错误，任一项缺失或类型不符返回 errPreflightFailed（reportExecuteError 放过哨兵不打印、main.go 转译退出码 1），作 CI/deploy 前置门禁；layoutByType map 驱动形态→骨架映射，layoutDSL/layoutService/layoutUI 为可组合单元，layoutEntry{path,dir} 区分目录/文件
preflight_test.go:   覆盖 runPreflight 的单元测试（fullstack 完整通过 / dsl 缺失 / service 缺 package.json / ui 缺 package.json / dsl 是文件非目录 / 空目录全失败 / service 型忽略缺失 ui 但仍要 service / ui 型忽略缺失 service / 未知 --type 报普通错误并点名 flag），用 t.TempDir 构造真实目录树隔离文件系统
...
diff.go:             diff 子命令，对比远端 Meta Server 上的 App DSL（Entity + Relation）与本地 YAML 文件的差异；App key 从 YAML 自动推断（Make.App key 或 Entity/Relation appKey 字段）；分页获取全部远端资源，按 Key 匹配后逐字段（key）/端点（entityKey）比对；支持 -f（必选）/ --output；退出码 0=无差异 1=有差异（table 与 json 模式一致，经 errDiffFound 哨兵上抛到 main 转译，不再用 os.Exit，可单测）；真实错误与 errDiffFound 哨兵均交由 reportExecuteError（errors.go 单一出口）处理——哨兵静默、真实错误打印
