...
// Package trust persists per-directory trust decisions so the makecli agent can
// avoid running side-effect tools (bash/write/edit) in directories the user has
// not trusted. Decisions are stored as a JSON map of directory path to a
...
// permission-hook integration live in the caller.
package trust
...
// DefaultPath returns the trust file location: <配置目录>/agent-trust.json，配置
// 目录沿用 makecli 惯例（internal/config.Dir：默认 ~/.make，受 MAKE_CLI_CONFIG_DIR
// 覆盖）。It returns "" when the config directory cannot be resolved, so a
// caller can treat trust as disabled rather than guessing a path.
func DefaultPath() string {
	dir, err := config.Dir()
	if err != nil {
		return ""
	}
	return filepath.Join(dir, "agent-trust.json")
}
...
// persisted: a future makecli launch re-prompts. Used by the "just once" REPL
...
// (mode 0o600, process-unique name) so two concurrent makecli processes writing
