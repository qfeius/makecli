...
// TestDefaultPath verifies DefaultPath honors MAKE_CLI_CONFIG_DIR (makecli 配置
// 目录惯例) and falls back to ~/.make/agent-trust.json.
func TestDefaultPath(t *testing.T) {
	t.Setenv(config.EnvConfigDir, "/custom/make")
	if got := DefaultPath(); got != "/custom/make/agent-trust.json" {
		t.Errorf("DefaultPath with MAKE_CLI_CONFIG_DIR = %q, want /custom/make/agent-trust.json", got)
	}
	...
	want := filepath.Join(home, ".make", "agent-trust.json")
