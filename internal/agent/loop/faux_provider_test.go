...
type fauxTurn []llm.AssistantMessageEvent

// textTurn scripts a turn that streams text as start → text delta → done(end_turn).
func textTurn(text string) fauxTurn {
	partial := core.AssistantMessage{RoleField: core.RoleAssistant}
	withText := partial
	withText.Content = core.ContentList{core.NewTextContent(text)}
	final := withText
	final.StopReason = core.StopReasonEndTurn
	return fauxTurn{
		llm.StreamStartEvent{Partial: partial},
		llm.StreamTextEvent{Partial: withText},
...
func toolCallTurn(id, name, args string) fauxTurn {
	partial := core.AssistantMessage{RoleField: core.RoleAssistant}
	withCall := partial
	withCall.Content = core.ContentList{core.NewToolCallContent(id, name, json.RawMessage(args))}
	final := withCall
	final.StopReason = core.StopReasonToolUse
	return fauxTurn{
		llm.StreamStartEvent{Partial: partial},
		llm.StreamToolCallEvent{Partial: withCall},
		llm.StreamDoneEvent{Message: final},
	}
}
...
func (p *fauxProvider) Models() []llm.Model { return p.models }
...
func (p *fauxProvider) requestAt(i int) llm.CompletionRequest {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.requests[i]
}

func (p *fauxProvider) StreamCompletion(ctx context.Context, req llm.CompletionRequest) (*llm.AssistantMessageEventStream, error) {
	p.mu.Lock()
	idx := p.calls
	p.calls++
...
func newFauxRunCfg(p *fauxProvider, tools ...core.AgentTool) RunConfig {
	reg := tool.NewToolRegistry()
	for _, tl := range tools {
		_ = reg.Register(tl)
	}
	return RunConfig{
		LoopConfig: LoopConfig{Model: "faux", Stream: llm.StreamFnFromProvider(p)},
...
func TestFauxProviderTextToolText(t *testing.T) {
	p := &fauxProvider{
		name:   "faux",
		models: []llm.Model{{Provider: "faux", ID: "faux"}},
		turns: []fauxTurn{
	...
	served := false
	cfg := newFauxRunCfg(p, echoTool("echo", core.ToolExecutionParallel, false))
	cfg.GetFollowUpMessages = func(ctx context.Context, agentCtx *core.AgentContext) []core.AgentMessage {
		if served {
			return nil
		}
		served = true
		return []core.AgentMessage{core.UserMessage{RoleField: core.RoleUser, Content: core.ContentList{core.NewTextContent("go on")}}}
	}
	agentCtx := &core.AgentContext{Messages: core.MessageList{core.UserMessage{RoleField: core.RoleUser, Content: core.ContentList{core.NewTextContent("start")}}}}
	...
	if kinds[0] != core.EventAgentStart || kinds[len(kinds)-1] != core.EventAgentEnd {
		t.Fatalf("run must be bracketed by agent_start/agent_end, got %v", kinds)
	}
	if countKind(kinds, core.EventMessageStart) < 3 || countKind(kinds, core.EventMessageEnd) < 3 {
		t.Errorf("expected fine-grained message deltas for each turn, got %v", kinds)
	}
	if countKind(kinds, core.EventMessageUpdate) < 3 {
		t.Errorf("expected message_update deltas (text/toolcall), got %v", kinds)
	}
	if got := countKind(kinds, core.EventToolExecutionStart); got != 1 {
		t.Errorf("expected 1 tool_execution_start, got %d in %v", got, kinds)
	}
	if got := countKind(kinds, core.EventToolExecutionEnd); got != 1 {
		t.Errorf("expected 1 tool_execution_end, got %d in %v", got, kinds)
	}
	if got := countKind(kinds, core.EventTurnStart); got != 3 {
...
func textContentOf(list core.ContentList) string {
	var s string
	for _, c := range list {
...
func TestFauxSeamSixHooks(t *testing.T) {
	...
	cfg := newFauxRunCfg(p, echoTool("echo", core.ToolExecutionParallel, false))
	cfg.Provider = "faux"
	cfg.TransformContext = func(ctx context.Context, msgs core.MessageList) core.MessageList {
		fired.transform = true
		return msgs
	}
	cfg.ConvertToLlm = func(msgs core.MessageList) core.MessageList {
		fired.convert = true
		return msgs
	}
	...
	stopCalls := 0
	cfg.ShouldStopAfterTurn = func(ctx context.Context, agentCtx *core.AgentContext) bool {
	...
	agentCtx := &core.AgentContext{Messages: core.MessageList{core.UserMessage{RoleField: core.RoleUser, Content: core.ContentList{core.NewTextContent("hi")}}}}
...
func TestFauxSeamTruncationProtection(t *testing.T) {
	...
	truncPartial := core.AssistantMessage{RoleField: core.RoleAssistant, Content: core.ContentList{core.NewToolCallContent("t1", "echo", json.RawMessage(`{}`))}}
	truncFinal := truncPartial
	truncFinal.StopReason = core.StopReasonLength
	p := &fauxProvider{
		name: "faux",
		turns: []fauxTurn{
			{
				llm.StreamStartEvent{Partial: core.AssistantMessage{RoleField: core.RoleAssistant}},
				llm.StreamToolCallEvent{Partial: truncPartial},
				llm.StreamDoneEvent{Message: truncFinal},
			},
			textTurn("recovered"),
		},
	}
	...
	var foundFail bool
	for _, m := range msgs {
		if tr, ok := m.(core.ToolResultMessage); ok && tr.IsError && tr.ToolCallID == "t1" {
			foundFail = true
		}
	}
	if !foundFail {
...
func TestFauxSeamParallelOrderingPreserved(t *testing.T) {
	...
	partial := core.AssistantMessage{RoleField: core.RoleAssistant, Content: core.ContentList{
		core.NewToolCallContent("a0", "slow", json.RawMessage(`{}`)),
		core.NewToolCallContent("a1", "mid", json.RawMessage(`{}`)),
		core.NewToolCallContent("a2", "fast", json.RawMessage(`{}`)),
	}}
	final := partial
	final.StopReason = core.StopReasonToolUse
	p := &fauxProvider{
		turns: []fauxTurn{
			{llm.StreamStartEvent{Partial: core.AssistantMessage{RoleField: core.RoleAssistant}}, llm.StreamToolCallEvent{Partial: partial}, llm.StreamDoneEvent{Message: final}},
			textTurn("done"),
		},
	}
