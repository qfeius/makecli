...
func (t *BashTool) ExecutionMode() core.ToolExecutionMode {
	return core.ToolExecutionSequential
}
...
func (w streamWriter) Write(p []byte) (int, error) {
	...
	w.mu.Unlock()
	if w.onUpdate != nil {
		w.onUpdate(core.AgentToolResult{Content: core.ContentList{core.NewTextContent(snapshot)}})
	}
	return len(p), nil
}
...
func (t *BashTool) Execute(ctx context.Context, id string, args json.RawMessage, onUpdate core.ToolUpdateFunc) (core.AgentToolResult, error) {
	...
	if runCtx.Err() == context.DeadlineExceeded {
		return core.AgentToolResult{Content: core.ContentList{core.NewTextContent(output)}},
	...
	if err != nil {
		...
		return core.AgentToolResult{
				Content: core.ContentList{core.NewTextContent(output)},
