...
type ToolExecutorConfig struct {
	...
	PrepareArguments core.PrepareArgumentsFunc
	BeforeToolCall   core.BeforeToolCallFunc
	AfterToolCall    core.AfterToolCallFunc
}
...
func executeToolCall(ctx context.Context, cfg ToolExecutorConfig, call core.AgentToolCall, emit core.EmitFunc) (core.ToolResultMessage, bool) {
	...
	// 2. execute.
	if emit != nil {
		if err := emit(ctx, core.ToolExecutionStartEvent{ToolCallID: call.ID, ToolName: call.Name, Args: args}); err != nil {
...
func prepareToolCall(ctx context.Context, cfg ToolExecutorConfig, call core.AgentToolCall) (core.AgentTool, json.RawMessage, *core.AgentToolResult, bool) {
	if ctx.Err() != nil {
	...
	if cfg.BeforeToolCall != nil {
		if dec := cfg.BeforeToolCall(ctx, core.AgentToolCall{ID: call.ID, Name: call.Name, Arguments: args}); dec != nil && dec.Block {
			r := core.AgentToolResult{}
			if dec.Content != nil {
				r.Content = *dec.Content
			} else {
				r.Content = core.ContentList{core.NewTextContent(fmt.Sprintf("tool %q blocked by beforeToolCall", call.Name))}
			}
			if dec.Details != nil {
...
func runTool(ctx context.Context, tool core.AgentTool, call core.AgentToolCall, args json.RawMessage, emit core.EmitFunc) (result core.AgentToolResult, isError bool) {
	defer func() {
		if r := recover(); r != nil {
	...
	onUpdate := func(partial core.AgentToolResult) {
		if emit == nil {
			return
		}
		_ = emit(ctx, core.ToolExecutionUpdateEvent{ToolCallID: call.ID, ToolName: call.Name, PartialResult: partial})
	}
...
func finalizeToolCall(ctx context.Context, cfg ToolExecutorConfig, call core.AgentToolCall, result core.AgentToolResult, isError bool, emit core.EmitFunc) (core.ToolResultMessage, bool) {
	...
	if emit != nil {
		_ = emit(ctx, core.ToolExecutionEndEvent{ToolCallID: call.ID, ToolName: call.Name, Result: result, IsError: isError})
	}
...
func errorResult(msg string) core.AgentToolResult {
	return core.AgentToolResult{Content: core.ContentList{core.NewTextContent(msg)}}
}
...
func decodeArgs[T any](args json.RawMessage, tool string) (T, *core.AgentToolResult) {
	var a T
...
func errorToolResult(call core.AgentToolCall, msg string) core.ToolResultMessage {
	return core.ToolResultMessage{
		RoleField:  core.RoleToolResult,
		ToolCallID: call.ID,
		ToolName:   call.Name,
