...
func runBash(t *testing.T, tool *BashTool, args map[string]any, onUpdate core.ToolUpdateFunc) (core.AgentToolResult, error) {
	t.Helper()
...
func TestBashToolStreaming(t *testing.T) {
	...
	var mu sync.Mutex
	var updates []string
	onUpdate := func(r core.AgentToolResult) {
		mu.Lock()
...
func TestBashToolMode(t *testing.T) {
	...
	if tool.ExecutionMode() != core.ToolExecutionSequential {
