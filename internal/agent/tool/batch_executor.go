...
func ExecuteToolCalls(ctx context.Context, cfg BatchConfig, calls []core.AgentToolCall, emit core.EmitFunc) ([]core.ToolResultMessage, bool) {
	if len(calls) == 0 {
		return nil, false
	}

	results := make([]core.ToolResultMessage, len(calls))
	...
	if cfg.ForceSequential || batchRequiresSequential(cfg.Registry, calls) {
		...
		for i, call := range calls {
			wg.Add(1)
			go func(i int, call core.AgentToolCall) {
				defer wg.Done()
				results[i], terminates[i] = executeToolCall(ctx, cfg.ToolExecutorConfig, call, emit)
			}(i, call)
		}
...
func batchRequiresSequential(reg *ToolRegistry, calls []core.AgentToolCall) bool {
	for _, call := range calls {
		if tool, ok := reg.Get(call.Name); ok && tool.ExecutionMode() == core.ToolExecutionSequential {
			return true
		}
	}
	return false
