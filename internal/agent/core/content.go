// Package core defines the core "leaf" data types and control flow for the
// makecli agent harness. It is the foundation package that every other agent
// sub-package depends on and imports nothing from them.
//
// 整包移植自 github.com/smallnest/pigo 的 internal/agentcore（MIT License,
// Copyright (c) 2026 smallnest），按 makecli 需要裁剪改写（保语义、改包路径）。
// pigo 自身是 pi agent loop 的 Go 重实现，原始注释保留。
//
