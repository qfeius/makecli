...
- `flow.go`: 登陆流程主原语，提供 Token / AuthorizationRequest / TokenExchangeRequest / CallbackServer 类型与 BuildAuthorizationURL（Resource 空则省略）/ ExchangeAuthorizationCode（authorization_code 换 token，解析 expires_in 为 Expiry）/ StartCallbackServer（绑定 127.0.0.1:0 空闲端口，返回 server + 动态 redirectURL；/callback 只认带 code 或 error 的真实回调，杂散空请求返 204 忽略，防 cap-1 channel 被空 state 毒化导致误报 state mismatch；成功页为极简 HTML 不泄露 URL）/ Wait（校验 state 取 code，支持 ctx 超时）/ OpenBrowser（跨平台打开 URL）
- `flow_test.go`: 覆盖 BuildAuthorizationURL query 断言 / ExchangeAuthorizationCode 表单与 Expiry / 回调服务器成功-state不匹配-超时三态 + 忽略杂散空请求回归，用 httptest + 真实 loopback server
