...
environment.go:      后端环境拓扑中枢，把 dev/test/production 三套**主机基址**（make/make-repo/myaccount，遵循 {dev-,test-,""} 前缀规律，均为 scheme://host 不含路径）收成一等 Environment preset；Meta/Repo 的网关前缀 /api/make 不在此、由 cmd 层 withGateway 补齐，Auth 基址供 login 追加 .well-known；提供 LookupEnvironment（空名回退 DefaultEnvironment=dev，未知名 ok=false）/ EnvironmentNames；作 cmd 层 URL 解析链的兜底层（flag > profile config > 环境 preset）
