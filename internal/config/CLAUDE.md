...
settings.go:         读取 config 文件 [settings] 全局段，提供 Settings 类型（CheckForUpdates *bool / Environment string）与 LoadSettings；settingsSection 常量由此定义，profile 解析层自动跳过该段；写入走 config.go 的 SetSetting
settings_test.go:    覆盖 LoadSettings（check-for-updates 三态 + environment 读取）+ SetSetting 写入并保留 profile/其他 settings 键 + LoadConfig 隔离 [settings] 段
