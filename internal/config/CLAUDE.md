...
config.go:           读写 config 文件（默认 ~/.make/config，INI 格式），提供 LoadConfig/SaveConfig/ConfigPath，Config/ConfigProfile 类型（含 XTenantID/OperatorID）；parseINISections 通用 INI 解析器供 settings.go 复用
