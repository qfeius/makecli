...
config.go:           读写 config 文件（默认 ~/.make/config，INI 格式），提供 LoadConfig/SaveConfig/ConfigPath，Config/ConfigProfile 类型（含 XTenantID/OperatorID）；SaveConfig 经 atomicWrite 原子落盘并保留 [settings] 段；parseINISections 通用 INI 解析器供 settings.go 复用
