...
integration.go:      Integration 服务调用层，提供 OCROptions 类型 + Client.OCR(filename, reader, opts) 方法；multipart 上传 file/business_id/verify_vat 到 /integration/v1/ocr；与 do() 同源注入 Traceparent/X-Log-Id（internal/trace）；6 个 query 参数（coord_restore/specific_pages/crop_complete_image/crop_value_image/merge_digital_elec_invoice/return_ppi）通过 OCROptions.queryString 序列化；data 字段以 map[string]any 透传给上层渲染
