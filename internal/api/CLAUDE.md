...
- `GetApp / GetEntity / GetRelation / GetBuildTask` 仅在资源"确实不存在"时返回（用 `%w` 包裹）：业务码 404，或 code==200 且主键为零值（服务端软空响应约定）
...
- `checkGetResult(code, msg, exists)` 是三态收敛 helper（存在/不存在→ErrNotFound/真实错误）；exists 由调用方按资源主键判定（Meta 资源 `data.key != ""`、构建任务 `data.id != 0`），字符串/数字主键统一收敛
...
- 识别点：`do()` 用轻量探针读 code（覆盖 Meta/Data/Repo 全部走 do 的方法）；`integration.go` 的 OCR 自发请求，单独以 `result.Code` 判定；`user.go` 的 GetUserInfo 走 GET，用接口自身的直白 401（`notLoggedInCode`）判定（探针也会先命中 990300403）。三处共用 `authFailedErr(code,msg)` 单一构造点（`%w` 包裹哨兵 + 保留原始 code/msg 供上层展示）
...
- `build.go`: 构建服务（make-build-service）查询层，提供 BuildTask 类型（id/appKey/deploymentVersion/environment/ref/commitSha/commitMessage/status/phase/image/errorCode/errorMessage/createTime/startTime/finishTime，只收录 CLI 呈现所需字段；ID 是 Build→Deploy 全链路关联键）、BuildStatusSuccess/Failed/Canceled 终态常量与 Finished()/Succeeded() 终态判定方法（状态机知识收口 api 层，cmd 不判等状态串；`app deploy --wait` 轮询的停止条件）、GetBuildTask(commitSha) 方法（POST /build/v1/build + X-Make-Target: MakeService.GetResource，body {commitSha} 精确查询；网关前缀 /api/make 由 cmd 层补齐）；经 checkGetResult(exists = data.id != 0) 收敛存在性，任务不存在返回 ErrNotFound；供 `app deploy --status/--wait` 用本地 HEAD sha 反查部署进度（服务端另有按任务 ID 的 MakeService.StatusResource 轻量状态接口，支持 tailLines 取运行中日志尾，CLI 轮询频率下暂无必要，留作日志流式的升级路径）
- `build_test.go`: 覆盖 GetBuildTask 的单元测试（请求 target/path/body 与字段解析 / 404 业务码与 200 软空响应均 ErrNotFound / 500 业务错误不映射 not-found / 传输错误）+ Finished/Succeeded 终态判定表测，用 httptest 隔离网络
...
- `user.go`: 用户身份查询层，提供 UserTenant{ID,TenantName} / UserInfo{ID,Name,Tenant,Valid} 类型、GetUserInfo() 方法（GET /user/v1/info + X-Make-Target: MakeService.GetResource，无 body；网关前缀 /api/make 由 cmd 层补齐）；未登录业务码 notLoggedInCode=401 → ErrAuthFailed（探针也先命中 990300403），非 200 → 通用「API 错误」；Valid 表示当前用户与租户关系是否有效
- `user_test.go`: 覆盖 GetUserInfo 的单元测试（成功解析/401 与 990300403 均返回 ErrAuthFailed/非 200 业务码报错/GET 不带 Content-Type/传输错误），用 httptest 隔离网络；测试数据已脱敏
