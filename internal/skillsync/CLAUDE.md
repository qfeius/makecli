...
- `sync.go`: 同步层——Skip 判断后前置 EnsureNpx 环境门禁（Skip 不要求 npx）；默认每次执行 `npx -y skills add qfeius/make-platform-skills --all -y`；Options 控制 Version/Skip，Result 给 cmd/update.go 渲染用户可见输出；runSkillsCommand 包级 seam 隔离 npx 副作用并带 3 分钟超时（syncTimeout 与 trimOutput 被 remove.go 复用）
