...
## 成员清单
sync.go:           Make platform skills 同步/清单/删除层，默认每次执行 `npx -y skills add qfeius/make-platform-skills --all -y`；Options 控制 Version/Skip，Result 给 cmd/update.go 渲染用户可见输出；runSkillsCommand 包级 seam 隔离 npx 副作用并带 3 分钟超时
