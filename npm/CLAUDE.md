...
## 成员清单
- `bin/`: 子目录，仅含 makecli.js——主包 `@qfeius/makecli` 的 bin 入口，按 process.platform/arch 拼出子包名，require.resolve 子包内 bin/makecli[.exe] 后 spawnSync 透传 argv 与退出码；子包缺失（--omit=optional 或无预编译平台）时给出重装/下载提示并退出 1
